...
import { clearAccessTokenCookieAdmin, getAccessTokenCookieAdmin } from "../../utils/cookie";
...
const Sidebar = () => {
  ...
  const dataSidebar = [
    ...
    {
      id: 2,
      icon_url: donationIcon,
      name: "Campaign",
      navigate_url: "/campaign",
    },
    // {
    //   id: 3,
    //   icon_url: historyIcon,
    //   name: "History",
    //   navigate_url: "/historyadmin",
    // },
  ];
  ...
  if (result) {
    ...
    api
      ...
      .then((response) => {
        ...
        clearAccessTokenCookieAdmin(response.message);
        navigate("/login");
      })
      .catch((error) => {
        console.log(error);
  ...
  return (
    <div className="px-8 py-10 h-screen flex flex-col justify-between sticky  top-0 shadow-md">
      <div className="flex flex-col gap-10">
        <h1 className="text-[36px] text-[#87255B] font-bold text-center">GiveHope</h1>
        <div className="flex flex-col gap-8">
          {dataSidebar.map(({ icon_url, id, name, navigate_url }) => (
            <NavLink to={navigate_url} className='outline-none'>
              {({ isActive, isPending }) => (
                <div
                  className={`${
                    isActive ? "bg-gradient-to-r from-[#B370B0] to-[#F2FFFC]" : ""
